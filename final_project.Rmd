...
```
The light green indicates the cluster of countries with higher levels of happiness, mainly composed of Western European countries, North America and Australia, which are rich countries. The medium green is on behalf of the cluster of nations with lowest levels of happiness, like Africa  where many poverty-stricken countries are located. Besides, the dark green represents those countries with medium degree of happiness, consisting of the middle-income countries, South America, East, Southeastern as well as Northern Asia and East Europe.
...
```
Cluster 1 and 3 have negative mean index, imply that those countries are less likely to implement rigorous closure policy. While cluster 2 countries are prone to enforce stern closure policy. 
...
```
Cluster 1 enjoys negative government response index, while cluster 2 and 3 countries' governments are more responsive compared with cluster 1 countries.
...
```
Cluster 1 have negative containment and health index while cluster 3 countries' mean index is also nearly 0. Compared with cluster 2 countries, they are less prone to provide additional health investment.


```{r}
p4 <- cluster_mean_graph(happi_2020_k2, happi_2020_k2$ESI)+ labs(y = "Economic Support Index")
```
Cluster 1 countries' economic support on citizens are relatively weak, while countries for cluster 2 and 3 more prone to provide economic support for citizens, especially for countries in cluster 3.
...
```
#Improvements
First, all the four indices related to COVID-19 are simple averages of the individual component indicators,which might arouse the problem that the significance of some indicators can not be represented through the calculation. Therefore, some indicators might need to be weighted.
Second, indicators here are ordinary, so that they fail to reflect their true intensity. For example, income support is also relative to the amount of support.
